...
def visualize_results(csv_path):
    ...
    total_shared_bike = df['optimal_shared_bike_count'].sum()
    shared_bike_pct_change = (total_shared_bike / total_ebike_orig) * 100 if total_ebike_orig != 0 else 0
    ...
    print(f"共享单车数量变化（占优化前电动车总量）: {shared_bike_pct_change:.2f}%")
    print("-" * 25 + "\n")
    ...
    # 平均变化条形图
    metrics = ['拥挤指数', '安全指数', 'CO$_2$排放', '载客量', '电动车数量', '共享单车数量']
    mean_changes = [
        df['cong_pct_change'].mean(),
        df['safety_pct_change'].mean(),
        df['co2_pct_change'].mean(),
        df['passenger_pct_change'].mean(),
        df['electric_pct_change'].mean(),
        shared_bike_pct_change
    ]
    colors = ['skyblue', 'lightgreen', 'salmon', 'gold', 'lightcoral', 'deepskyblue']
    ...
    change_columns = [
        ...
        'safety_pct_change',
        'co2_pct_change',
        'passenger_pct_change',
        'electric_pct_change'
    ]
    # 共享单车数量变化方向（全为正或零，按总量计）
    shared_bike_positive = 1 if total_shared_bike > 0 else 0
    shared_bike_no_change = 1 if total_shared_bike == 0 else 0
    shared_bike_negative = 0  # 不考虑负数
    ...
    # 共享单车数量变化方向
    positive_counts.append(shared_bike_positive)
    negative_counts.append(shared_bike_negative)
    no_change_counts.append(shared_bike_no_change)
